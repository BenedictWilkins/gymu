...
def episode(env, model):
    ...
    obs = env.reset()
    observations.append(np.concatenate(obs[0].numpy(), axis=1))
    done = False
    while not done:
        action = model(obs.to(model.device))  
        #action = action.argmax().item()
        action = torch.multinomial(torch.sigmoid(action), 1).item()
...
def wrap_for_immitation_learning(env):
...
class Monitor(gym.Wrapper): # used for raw frames
    ...
    def step(self, action):
        ...
        if self.record is not None:
            self.record[-1][0].append(obs)
            self.record[-1][1].append(reward)
            self.record[-1][2].append(action)
            
        return (obs, reward, done, *rest)
...
def wrap_deepmind(env, episode_life=False, clip_rewards=True, frame_stack=4, scale=True, record=None):
    """
